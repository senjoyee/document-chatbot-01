...
1. Create a new Next.js project:
   ...
   ```
   npx create-next-app@latest uploadservice
   ```
...
6. Add the required shadcn/ui components:
   ...
   ```
   npx shadcn@latest add button card table checkbox
   ```

7. Create a new file for the chatbot components:

   create a file `uploadservice.tsx` under app\components

8. Open `app\components\uploadservice.tsx` in your code editor and paste the uploadservice component code.

9. Edit the `app/page.tsx` file to use the uploadservice.
    
10. Set up the Sonner toast component:
Create a new file `app/providers.tsx` and update it.

11. Update the root layout to use the Providers:
Edit the `app/layout.tsx` file
